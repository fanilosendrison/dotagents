...
Items qui necessitent un arbitrage humain avant d'etre traduits en fix atomique. Ces items ne sont **pas** traites par `/backlog-crush` ou `/backlog-deep-crush`. Voir `~/.agents/skills/fix-or-backlog/SKILL.md` pour la convention de format et la logique d'escalade auto.
...
cmd_escalate_stuck() {
	...
	while IFS=$'\t' read -r id count; do
...

		{
			...
			echo "  - cta: examine manually. See \`.agents/run/$SCRIPT_NAME/*/\` for sub-agent skip reasons."
			echo ""
		} >> "$design_file"
