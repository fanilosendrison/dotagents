...
async function main() {
	...
	const { action, content, date, title, topic, trigger, wiring } = input;
	const harnessDirectory = join(homedir(), ".agents");
	...
	writeFileSync(
		indexPath,
		indexContent +
			formatIndexEntry(nextIndex, title, date, topic, wiring, trigger),
		"utf8",
	);
	writeOutput(`✓ docs/CONTEXT.md (entry ${nextIndex})`);

	const routerPath = join(harnessDirectory, "AGENTS.md");
	try {
