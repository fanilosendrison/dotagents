...
_cleanup_old_runs() {
	local base="$LOOP_CLEAN_REPO_ROOT/.agents/run/loop-clean"
	[[ -d "$base" ]] || return 0
...
cmd_init() {
	...
	local run_dir="$repository_root/.agents/run/loop-clean/$session_id"
	...
	rm -f "$baseline_tmp"
	if ! GIT_OPTIONAL_LOCKS=0 git -C "$repository_root" check-ignore -q -- ".agents/run/loop-clean/$session_id"; then
		cat >&2 <<'WARNING'
WARNING: .agents/run/ is not ignored at the resolved Git root.
Add .agents/run/ to the root .gitignore before committing.
WARNING
	fi
