...
import { phaseInputSchema } from "../schemas.ts";
import type { PhaseInput, PreflightResult } from "../types.ts";
import {
  runGitCommand,
...
export async function runInputPreflight(
  ...
  try {
    const parsed = safeParsePhaseInput(input);
    if (!parsed.ok) {
    ...
    if (!stableIdentifierPattern.test(parsed.input.phase)) {
      return fail("phase must be a stable ASCII identifier");
    }
...
function safeParsePhaseInput(
  input: PhaseInput,
): { ok: true; input: PhaseInput } | { ok: false } {
  try {
