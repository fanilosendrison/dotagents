...
import {
  invalidEvidenceRefStage,
  passingStageWithEvidence,
} from "../fixtures/stages.ts";

describe("runStage properties", () => {
  ...
  test("P4 preflight failures never invoke stage", async () => {
    const repo = await createCommittedRepo();
    const artifacts = await createArtifactParent();
    const badInputs: StageInput[] = [
      { ...buildInput(repo.workDir, artifacts.artefactDir, repo.baseSha), runId: "bad/run" },
      { ...buildInput(repo.workDir, artifacts.artefactDir, repo.baseSha), stage: "" },
      ...
      try {
        const stage: Stage = async () => {
          ...
          return {
            ...
            errors:
              status === "failed"
                ? [{ message: "stage issue", severity: "minor" }]
                : [],
          };
        };
        const output = await runStage(
