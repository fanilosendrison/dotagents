...
- Résoudre `canonicalRepositoryRoot` depuis le `RepoCapture`.
- Si aucun dépôt Git n'existe à `canonicalRepositoryRoot` :
  - L'initialiser (`git init`), ajouter les fichiers existants (`git add -A`) et
    auto-committer cet état existant comme commit initial (plutôt qu'un commit
    vide). Cela évite de transformer le code existant en dirty state non géré.
  - Créer un repo distant via l'API du provider configuré dans
    `ProviderConfig` (nom = `basename(canonicalRepositoryRoot)`,
    visibilité = `ProviderConfig.defaultVisibility`).
  - Ajouter le remote `origin` (`git remote add origin <url>`).
  - Pousser le commit initial (`git push -u origin main`).
...
- Déterminer `baseHeadSha` : `git rev-parse HEAD` dans le checkout source.
- Déterminer `baseBranch` : `git rev-parse --abbrev-ref HEAD` dans le checkout
  source. Si HEAD est détaché, `baseBranch` vaut `"(detached)"` et le run
  procède normalement (la branche `work/<runId>` est créée depuis `baseHeadSha`,
  pas depuis `baseBranch`).
- Déterminer `defaultTargetBranch` : `git symbolic-ref refs/remotes/origin/HEAD`
  dans le checkout source, en extrayant le nom court de branche (ex:
  `refs/remotes/origin/main` → `main`). Si `origin/HEAD` n'est pas configuré,
  `workspace-setup` échoue avec `failed` et demande à l'utilisateur d'exécuter
...
- **HEAD détaché** : `baseBranch` vaut `"(detached)"`. `baseHeadSha` reste le
  SHA du commit détaché. La branche `work/<runId>` est créée normalement depuis
  ce SHA.
- **Pas de remote `origin`** : `workspace-setup` échoue avec `failed` (sauf si
  elle vient d'initialiser le dépôt, auquel cas elle configure `origin`
  automatiquement via `ProviderConfig`).
- **`origin/HEAD` non configuré** : `workspace-setup` échoue avec `failed`. Le
  message d'erreur indique la commande `git remote set-head origin --auto`.
- **Échec de création du repo distant** : `errored`. L'état local (commits)
...
if-is-new-repository-initialize-git-repo-and-remote
  git-init
  git-add-all-and-commit
  create-remote-repo-via-provider-api
  git-remote-add-origin
  git-push-initial-commit
verify-repo-capture-against-git
validate-dirty-state-policy
determine-base-head-sha (git rev-parse HEAD)
determine-base-branch (git rev-parse --abbrev-ref HEAD)
resolve-default-target-branch (git symbolic-ref refs/remotes/origin/HEAD)
...
- `ProviderConfig` absent alors qu'un `git init` est nécessaire : `errored`.
- Racine Git reelle differente de `canonicalRepositoryRoot` : `failed`.
- `projectRoot` hors repo : `failed`.
- `origin/HEAD` non configuré sur un dépôt existant : `failed`.
- Création du repo distant échouée (API provider injoignable, token invalide,
  nom déjà pris) : `errored`.
