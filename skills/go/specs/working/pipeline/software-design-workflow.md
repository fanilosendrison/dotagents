...
`/go` est un pipeline déterministe qui encadre des moments non déterministes.

Le pipeline ne crée pas le changement par lui-même. Il prépare, délègue,
...
Le pipeline vérifie ce changement par des artefacts, des gates, des findings, et
des hashes.
...
Le pipeline commence par figer la demande. Sans cela, l'agent peut corriger un
...
Le point de départ Git doit être figé avant toute mutation. Le pipeline crée un
worktree physique privé pour éviter les collisions entre sessions et les dirty
states partagés.

### `project-discovery`

Le pipeline ne doit pas inventer les commandes de check. Il détecte ce que le
...
Après l'agent, le pipeline capture l'état réel. Ce snapshot est la frontière
...
Si la review trouve des risques bloquants, le pipeline demande une décision ou
délègue une correction approuvée. Toute correction invalide les checks
précédents.

### `package-plan`

Une fois le résultat global conforme, le pipeline découpe le diff final en
paquets logiques.
