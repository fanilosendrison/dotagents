...
La FSM `commit-push-pr` refuse de démarrer si l'une de ces conditions manque :
...
- Le checkout courant est `work/<run-id>`, ou le pipeline sait le recréer depuis
  `baseHeadSha`.
- Le diff brut est capturé avec un hash canonique (`originalDiffHash`).
- `agent-conduct-check`, `lint`, `typecheck`, et `tests` sont passés sur ce hash.
...
- Chaque fichier modifié, supprimé, renommé, généré, binaire, ou non suivi est
  assigné à au moins un paquet.
...
3. Commit(s) atomique(s) avec message formaté (conventional commits)
...
```bash
# Push uniquement les branches de PR
git push origin pr/<run-id>/*
...
L'ouverture des PRs déclenche le worker CI qui reprend la FSM Turnlock pour la
phase `pr-ci-review`. Cette phase rejoue les gates mécaniques et la review
structurée sur le diff réellement poussé, pas sur le workspace local. Voir
`ideal-review.md` et `go-pipeline-contract.md`.
...
La phase commit/push/PR est une FSM Turnlock dans la FSM maîtresse `/go` :
...
Le `state.json` de la FSM commit-push-pr contient le plan de découpage, la preuve
de reconstruction du diff, le `WorkSession` produit par `workspace-setup`, les
branches de PR créées, les commits, les URLs des PRs ouvertes, les statuts de
merge, et le lien vers les artefacts de review. En cas d'interruption, la reprise
lit `state.json` et sait exactement où elle en est.
...
- `agent-conduct.md` — règles de conduite de l'agent pendant l'implémentation
- `go-pipeline-contract.md` — contrat des phases, sévérités, artefacts JSON, et gates
