...
This NIB-T defines RED behavioral tests for the standalone `/go` stage harness.
The tests verify the observable behavior of `runStage`: returned `StageOutput`,
...
```ts
import { runStage } from "../../src/stage-harness";
```
...
for the stage artefact directory.
...
```ts
type StageFixture = (input: StageInput) => Promise<StageDraftOutput>;
```

Provide these stage fixtures:
...
- `throwingPhase`: throws `new Error("stage exploded")`.
...
- Parses JSON.
- Asserts parsed JSON deep-equals the returned `StageOutput`.
...
### A1 - Passing stage writes canonical output
...
- `runStage(stage, input)` is called.
...
### A2 - Failing stage preserves stage errors
...
- Returned `errors` has exactly one stage error.
- The stage error has `severity: "minor"`.
- The stage error keeps `file`, `line`, and `evidenceRef`.
- `output.json` exists and matches the returned output.
- Canonical fields are non-null.

### A3 - Skipped stage has no errors
...
### A4 - Throwing stage becomes errored output
...
- At least one error message contains `stage exploded`.
- `output.json` exists and matches the returned output.
...
- Parsed `output.json` deep-equals the returned output.
- The stage-written placeholder content is not present.
...
- A stage function that increments an invocation counter if called.

Then:

- `runStage` rejects with a preflight/setup error.
- The stage invocation counter remains `0`.
- No `output.json` exists at the requested artefact path.

### A10 - Existing artefact directory fails before stage invocation
...
- Returned `status` is `errored`.
- The original stage error is preserved.
- The invalid `errors[].evidenceRef` is omitted from the returned stage error.
...
- Returned `status` is `errored`.
- The original stage error is preserved as much as possible.
- Invalid `file` or `line` metadata is omitted from the returned stage error.
...
- Returned `status` is `errored`.
- At least one blocking error names a reserved-file violation.
- The reserved file exists only as stage-created diagnostic residue; it is not
  trusted as harness output.
- `output.json` exists and matches the returned output.

### A17 - Missing artefact parent fails before stage invocation
...
- `runStage` rejects before returning `StageOutput`.
- The stage invocation counter remains `0`.
...
- Before `runStage`, replace `src/a.txt` on disk with a symlink.
- `passingPhaseWithEvidence`.
...
Run a stage that returns that reference.
...
### P4 - Preflight failures never invoke stage
...
Run with a stage that increments an invocation counter.

Assert:

- `runStage` rejects before returning `StageOutput`.
- The stage invocation counter is `0`.
- No canonical `output.json` exists.

### P5 - Output JSON is the canonical return value

Across all acceptance fixtures that produce a `StageOutput`, assert:
...
### P7 - Harness errors dominate stage status

For each stage draft status:

- `passed`
- `failed`
- `skipped`

Inject one harness-side validation or state error after stage invocation.

Assert:

- Returned `status` is `errored`.
- Stage errors, when valid, are preserved.
...
Apply these invariants to every acceptance test that returns `StageOutput`:

- `output.runId` equals input `runId`.
- `output.stage` equals input `stage`.
...
- The stage never writes the final canonical `output.json`; the persisted file
  always equals the harness return value.
- Root `stdout.txt` and `stderr.txt`, when stage-created, are never treated as
...
- Public export existence for `runStage`.
- Public export existence for TypeScript types.
- Direct schema shape tests that do not execute `runStage`.
...
- `DC-ZOD` for validation failure semantics visible through `runStage`.
