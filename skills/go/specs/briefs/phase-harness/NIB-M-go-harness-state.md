---
id: NIB-M-GO-HARNESS-STATE
type: nib-module
version: "1.0.0"
scope: go-phase-harness/state
status: active
consumers: [codex]
superseded_by: []
---
...
This module collects canonical repository state after a phase returns or throws.
...
The module does not decide final phase status. It reports unavailable canonical
...
Each `null` field must correspond to at least one blocking `PhaseError`.
...
On failure, return `null` and append a blocking `PhaseError` with a message that
names `headShaAfter`.
...
On command failure, return `null` and append a blocking `PhaseError` with a
...
- `HEAD` resolves to `<commit-after-phase>`.
...
`runPhase` calls M4 immediately after M3:
