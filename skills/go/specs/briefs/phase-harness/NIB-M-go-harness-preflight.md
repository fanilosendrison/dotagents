---
id: NIB-M-GO-HARNESS-PREFLIGHT
type: nib-module
version: "1.0.0"
scope: go-phase-harness/preflight
status: active
consumers: [codex]
superseded_by: []
---
...
This module validates and canonicalizes `PhaseInput` before a phase is invoked.
It rejects inputs and repository states that would make the harness output
ambiguous, non-portable, or non-deterministic.

Preflight is outside the `PhaseOutput` contract. A preflight failure produces no
...
The `input` object has already passed the structural `phaseInputSchema` from
...
if (!parsed.success) {
  return fail("PhaseInput failed schema validation");
}
```
...
Validate `runId` and `phase` with this exact pattern:
...
- Preflight must not emit `PhaseOutput`.

---

## 8. Integration

`runPhase` calls preflight before any other module:
