...
│                                                          │
│  Chaque /go exécute son propre pipeline (FSM Turnlock).   │
│  Conduct → lint → typecheck → tests → review → commit.   │
...
Chaque pipeline est une FSM Turnlock indépendante avec son propre `state.json`.
Les sessions ne communiquent pas entre elles. Le pipeline ne dépend pas de l'état
d'une autre session — il ne dépend que du `state.json` de sa propre run.
...
| Pipeline FSM par session | Sans Turnlock, le pipeline est dans la tête de l'agent ou dans un script ad-hoc. Pas de `state.json` portable. |
