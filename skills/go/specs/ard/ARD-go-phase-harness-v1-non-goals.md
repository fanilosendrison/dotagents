---
id: ARD-GO-PHASE-HARNESS-V1-NON-GOALS
type: ard
version: "1.0.0"
...
# ARD — Non-goals de la v1 du phase harness

## Contexte

Le [NIB-S du phase harness](../briefs/phase-harness/NIB-S-go-phase-harness.md)
...
resume). Le harness est conçu comme **Turnlock-free** — la phase est une
fonction asynchrone pure (`PhaseInput → PhaseDraftOutput`) qui ne connaît ni
Turnlock ni les state machines.

**Raison** : Séparation architecturale. Turnlock viendra comme un *wrapper
mécanique* autour de `runPhase`, sans modifier l'implémentation des phases. Si
le harness intégrait Turnlock en v1, il deviendrait couplé à un composant qui
lui est orthogonal.

**Trajectoire** : v2 — adaptateur Turnlock qui wrappe `runPhase` sans toucher au
harness.
...
**Nature** : L'API canonique est la function API (`Phase → runPhase →
PhaseOutput`). Un adaptateur CLI (subprocess, stdin/argv → stdout) est une
interface alternative, pas une partie du harness.

**Raison** : Le CLI est un consommateur du harness. L'ajouter en v1 mélangerait
deux responsabilités (exécution de phase vs interface de lancement) et
complexifierait le contrat fonctionnel. De plus, la capture stdout/stderr n'est
pas garantie pour les phases in-process — le CLI est le moment où cette capture
devient pertinente.

**Trajectoire** : v2 — thin wrapper qui lit `PhaseInput` depuis stdin/argv,
invoque `runPhase`, et écrit `PhaseDraftOutput` sur stdout.

---

## Décision 3 : Pas de chaînage de phases

**Nature** : Le harness exécute **une phase unique**. Le chaînage (output d'une
phase → input de la suivante, `previousPhaseOutputs`, `attemptNumber`) n'existe
pas.

**Raison** : Le chaînage est une responsabilité d'orchestration (le pipeline
`/go`), pas du harness. Le harness doit rester une brique pure : une phase, un
run, un `output.json`. Le pipeline assemble les briques.

**Trajectoire** : v2 — le pipeline `/go` passe `previousPhaseOutputs` et
`attemptNumber` dans `PhaseInput.config` ou dans un type enrichi, sans changer
le harness.
...
**Nature** : Aucun `timeoutMs` dans `PhaseInput`, aucun `AbortSignal`. Le
harness attend indéfiniment que la phase retourne ou throw.
...
de la complexité — que faire d'une phase interrompue ? Le `PhaseOutput` est-il
valide ? L'`artefactDir` est-il dans un état cohérent ? — sans bénéfice immédiat
pour valider le flux nominal.

**Trajectoire** : v2 — `timeoutMs` dans `PhaseInput` et `AbortSignal` pour
l'annulation coopérative. Une phase timeoutée → `errored`.
...
peut théoriquement perdre l'output malgré un `runPhase` réussi.
...
phase dont l'output a été perdu), mais ce n'est pas équivalent — rejouer une
phase réussie peut avoir des effets de bord. La garantie `fsync` reste
souhaitable en complément, pas en remplacement.

**Trajectoire** : v2 — spécification précise des appels `fsync` dans
`NIB-M-GO-HARNESS-PERSISTENCE` pour une durabilité Turnlock-grade.
...
`PhaseInput.config`, mais probablement pas nécessaire.
...
preflight (M1) bloque donc : si l'un de ces bits est détecté, la phase ne
démarre pas.
...
## Décision 8 : Pas de capture stdout/stderr garantie pour les phases in-process

**Nature** : Pour les phases exécutées comme fonctions asynchrones
...
qui écrivent sur stdout). Les phases doivent écrire leur diagnostic dans
