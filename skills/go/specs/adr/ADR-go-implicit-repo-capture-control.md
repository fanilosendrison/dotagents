---
id: ADR-GO-IMPLICIT-REPO-CAPTURE-CONTROL
type: ard
version: "1.0.0"
scope: go-workflow/repo-capture
status: active
supersedes: []
superseded_by: []
---
...
Le démarrage d'un run `/go` nécessite de résoudre le `RepoCapture`, c'est-à-dire déterminer sur quel dépôt Git l'agent doit s'ancrer, et à partir de quelle branche il doit créer son worktree de travail.
...
Le `RepoCapture` est désormais résolu de manière strictement implicite et déterministe via l'état du terminal au moment de l'invocation, selon la philosophie : **"là où tu es, là où tu bosses"**.
...
- **Simplicité du contrat** : Le schéma JSON du `RepoCapture` perd des champs redondants (`baseBranchHint`).
