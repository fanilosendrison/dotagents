...
| État terminal binaire | DONE (check passé) ou FAIL (tous les budgets épuisés) |
| Artefact JSON | Résultat validé par schéma, persisté dans `state.json` |
...
Toutes les étapes partagent la même infrastructure. Mais toutes les phases ne
sont pas des FSMs imbriquées — seules celles qui nécessitent un cycle check/fix/retry.

### Deux types de phases

| Type | Exemples | Mécanisme Turnlock |
|------|----------|-------------------|
...
│    └── DONE / REMEDIATE / FAIL → transition    │         │
│                                           └────┘         │
