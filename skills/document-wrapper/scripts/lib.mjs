...
export function computeNextIndex(indexContent) {
...
export function formatIndexEntry(num, title, date, topic) {
	return (
		`\n### ${num}. ${title}\n` +
		`- **Date** : ${date}\n` +
...
export function formatQuickNavRow(action, topic, description) {
...
export function findDocsSection(lines) {
	let start = -1;
	let end = -1;
 ...
 */
export function parseDocEntries(sectionLines) {
	const header = [];
	const entries = [];
	let i = 0;
	...
	while (i < sectionLines.length) {
		if (
			/^│ {3}[├└]── .+\.md\s*(?:←|$)/.test(sectionLines[i]) &&
			!sectionLines[i].includes("CONTEXT.md")
		)
			break;
 ...
 */
export function buildDocTree(docLine, header, entries) {
	const rebuilt = [];
	let maxNameLen = 0;
 ...
 */
export function insertFolderEntry(routerContent, topic, description) {
