...
outputs:
  - path: "$RUNDIR/structural-report.md"
    format: "markdown-structural-report"
dependencies:
  - "~/.agents/conventions/cubits-design-doc.md"
  - "assets/cdd-structure-reference.md"
unlock_condition: "Step 07 is CLEAN (every requirement classified)"
---

# Step 8: Structural Completeness

Lis `$RUNDIR/behavioral-report.md` pour connaître l'état du step 07.
...
This step only executes if **Step 07 (Behavioral) is CLEAN**. If Step 07 is blocked, do not execute. Report: *"Structural audit blocked: Step 07 (Behavioral) is not clean."*
...
- **Detect loop-back need:** if factorization proposes a split (CDD → CDD-O + CDD-N), flag it: *"DAG split proposed. After applying the split, loop back to Step 02 (Sniper Run) on the new CDDs."*
...
## Loop-Back Alerts
- ⚠️ **DAG split on CDD-CHECKOUT** → after applying split, re-run Step 02 (Sniper Run) on new CDD-O and CDD-N documents.

## Step 8 Verdict: CLEAN / BLOCKED

- CLEAN: correctly factored, all boundaries typed → proceed to Step 9.
