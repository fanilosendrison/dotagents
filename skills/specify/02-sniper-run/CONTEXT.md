...
outputs:
  - path: "$RUNDIR/sniper-<document>.json"
    format: "sniper-json"
---

# Step 2: Sniper Run

For each `$RUNDIR/requirements-<filename>.json`, launch the 8 sub-agents in parallel with `context: fresh`:
...
**Output**: `$RUNDIR/sniper-<document>.json`.
