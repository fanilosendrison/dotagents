...
outputs:
  - path: "$RUNDIR/coherence-report.md"
    format: "markdown-coherence-report"
dependencies:
  - "~/.agents/conventions/cubits-design-doc.md"
unlock_condition: "Step 08 is CLEAN (correctly factored, all boundaries typed)"
---

# Step 9: Inter-CDD Coherence

Lis `$RUNDIR/structural-report.md` pour connaître l'état du step 08.
...
This step only executes if **Step 08 (Structural) is CLEAN**. If Step 08 is blocked, do not execute. Report: *"Coherence audit blocked: Step 08 (Structural) is not clean."*
...
**Context:** Fresh. Provide the entire `/specs` corpus and the Step 02 behavioral report (for `SPEC_CONFLICT` cross-references).
...
Collect the sub-agent's findings. Cross-reference any `SPEC_CONFLICT` flags from Step 02 that pointed to inter-CDD issues — verify they are resolved or confirmed by the Coherence step.
...
- CLEAN: all contracts match, vocabulary consistent, no violations → proceed to Step 10.
