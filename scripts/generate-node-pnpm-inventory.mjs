		...
		exclusions: [
			{
				...
				category: "archive",
				reason:
					"Archived implementations are retained as historical evidence and are not executable migration surfaces.",
			},
			{
				pattern: "skills/go/specs/legacy/**",
				category: "archive",
				reason:
					"Legacy design material is retained without silent historical rewriting.",
			},
			{
				pattern:
					"skills/go/specs/briefs/orchestrator/DC-BUN-SPAWN-ASYNC-RUNTIME.md",
				category: "superseded-specification",
				reason:
					"Bun-specific runtime clauses are superseded by the Node runtime successor contract.",
			},
			{
			...
			{
				pattern:
					"skills/go/specs/briefs/orchestrator/NIB-M-GO-ASYNC-GIT-RUNNER.md",
				category: "superseded-specification",
				reason:
					"Bun-specific runtime clauses are superseded by the Node runtime successor contract.",
			},
			{
				pattern:
					"skills/go/specs/briefs/orchestrator/NIB-S-GO-TURNLOCK-ORCHESTRATOR.md",
				category: "superseded-specification",
				reason:
					"Only Bun-specific implementation clauses are superseded; business behavior remains authoritative.",
			},
			{
				pattern:
					"skills/go/specs/briefs/orchestrator/NIB-T-GO-TURNLOCK-ORCHESTRATOR.md",
				category: "superseded-specification",
				reason:
					"Only Bun test-infrastructure clauses are superseded; behavioral vectors remain authoritative.",
			},
			{
				pattern:
					"skills/go/specs/briefs/orchestrator/PLAN-GO-TURNLOCK-ORCHESTRATOR-PHASE-1.md",
				category: "superseded-specification",
				reason:
					"Bun-specific construction guidance is superseded by the Node runtime successor contract.",
			},
			{
				pattern: "skills/go/specs/working/orchestrator/turnlock-bridge.md",
				category: "superseded-specification",
				reason:
					"Bun-specific bridge runtime guidance is superseded by the Node runtime successor contract.",
			},
		],
	},
	dotpi: {
		...
		exclusions: [
			{
				...
				category: "upstream",
				reason:
					"The upstream pi-subagents-4-turnlock extension remains outside the migration scope.",
			},
		],
	},
};
...
function collectBunTestSurfaces(records, repository, exclusions) {
	return records
		...
		.map((record) => ({
			...
			role: record.repositoryPath.endsWith(".test.ts")
				? "test"
				: "assertion-helper",
			testCaseNames: extractQuotedTestCases(record.contents),
			tableDrivenDeclarations: [
				...record.contents.matchAll(/\b(?:test|it)\.each\s*\(/g),
			].length,
...
async function main() {
	...
	await writeFile(
		...
		`${JSON.stringify(parityManifest, null, 2)}\n`,
	);
}
