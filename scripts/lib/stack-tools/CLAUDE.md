...
└── src/
    ...
    └── __tests__/             # node:test files mirroring lib module by module
```
...
- **Shared runtime**: uses the repository Node runtime for subprocesses and fixed-schema YAML parsing, plus `node:fs` for file access.
