 ...
 *
 * Usage:
 *   bun ~/.agents/scripts/claude-facade/src/cli.ts install [--repair] [--agents-root=<path>] [--claude-root=<path>]
 *   bun ~/.agents/scripts/claude-facade/src/cli.ts check    [--agents-root=<path>] [--claude-root=<path>]
 *   bun ~/.agents/scripts/claude-facade/src/cli.ts gitignore-rules
 */

import { checkAll, installAll, generateGitignoreRules } from "./facade";
...
function parseArgs(args: string[]): {
  ...
  for (const arg of args) {
    ...
    else if (arg.startsWith("--agents-root=")) agentsRoot = arg.slice("--agents-root=".length);
    else if (arg.startsWith("--claude-root=")) claudeRoot = arg.slice("--claude-root=".length);
  }

  if (!command) {
    console.error("Usage: cli.ts <install|check|gitignore-rules> [--repair] [--agents-root=<path>] [--claude-root=<path>]");
    process.exit(2);
  }
...
function main() {
  const { command, repair, agentsRoot, claudeRoot } = parseArgs(process.argv.slice(2));
