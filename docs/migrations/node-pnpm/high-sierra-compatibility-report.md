...
- The active `start` script now delegates to a Node launcher that compiles the shared runtime and untracked skill artifacts with `process.execPath`, then owns the compiled supervisor as an isolated signal-forwarded child. The real bare-remote test runs through this launcher and confirms no internal Bun, pnpm, or shell pipeline. Node run `33257640938` and retained Bun run `33257640937` pass on Linux/macOS; two frozen installs in spaces/Unicode paths keep all lockfiles stable and the Bun sentinel empty.
- Real self-hosting run `01M16Z8A030MJZAHMZX554CJPM` used that active launcher to validate two repositories, delegate through the live DeepSeek boundary, create dotagents commit `10b5da5` and dotpi commits `5990b33` plus `3c1cf95`, and push both branches. Its terminal event is successful, the persisted run is classified `drained`, both repositories are clean and synchronized, and Node/Bun CI runs `33258149595`, `33258149602`, `33258151521`, and `33258151486` pass. The direct-Git bootstrap exception is therefore closed.
- The `scripts` package preserves all 16 retained test surfaces under a closed Node runner: 141 tests across 40 suites pass with strict Node typecheck, three public CLI smokes, shared subprocess/YAML primitives, and no production Bun API. Two frozen installs in spaces/Unicode paths preserve pnpm lock SHA-256 `24963862168dec1ddc1dfe7e39d95ec2f54fadacbd135688f5af748db5269b16` and leave the Bun sentinel empty. Node run `33271564980` and Bun run `33271564979` prove differential parity; after removal of the package-local Bun lock and configuration, cutover runs `33271871874` and `33271871906` pass on Linux/macOS. The retained Bun baseline now consumes pnpm-locked dependencies.
- The self-contained `loop-clean/protocol` package runs its controller and adjacent TypeScript CLI through Node while preserving exact read-only Git, scope, routing, runtime-gate, HEAD, and index observables. Its closed runner passes all 87 tests across 11 suites; strict typecheck and all 9 named mutants pass, including autonomous installation and the Git 2.17-compatible staged/worktree vector. Two frozen installs in spaces/Unicode paths preserve pnpm lock SHA-256 `44324354132ed2957ec49d22a88de756da5752e7cb160b9db51ac7f0e7ff6bf2` and leave the Bun sentinel empty. Differential runs `33302922515` and `33302922514` passed before removal of the package-local Bun lock/configuration; cutover runs `33303609674` and `33303609630` pass on Linux/macOS.
- The `go` stage harness keeps production sources unchanged while its two closed Node test files and shared assertion helper preserve all 26 acceptance cases and 8 property cases. Exact `fast-check@3.23.2`, `numRuns: 8`, default seed selection, emitted counterexample seeds, and shrink paths remain intact. Typecheck/lint, 34 tests, and two frozen installs in spaces/Unicode paths pass with pnpm lock SHA-256 `a1f24bc10772b2387d819ad27cfa9c410205e8b39b1fbf13e0c66baa8eca160d` and an empty Bun sentinel. Differential runs `33304351573` and `33304351543` passed before removal of `skills/go/bun.lock`; cutover runs `33304638852` and `33304638836` pass on Linux/macOS.
